...
네트워크와 서버 인프라를 처음으로 직접 설계하고 구축한 프로젝트였습니다. 팀장으로서 전체 일정과 방향을 이끌어가는 것이 낯설었지만, 이전 직장에서 네트워크 설계도를 접했던 경험을 바탕으로 토폴로지 설계와 문서화 작업을 주도할 수 있었습니다.
서버 구축을 함께 담당한 팀원과 긴밀하게 협력하며 계획부터 구축까지 전 과정을 수행했고, 짧은 기간이었지만 실제 RACK 구성부터 인프라 전반의 흐름을 직접 만들어본 값진 경험이었습니다.
